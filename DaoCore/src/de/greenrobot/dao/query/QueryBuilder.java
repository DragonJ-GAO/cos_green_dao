...
public class QueryBuilder<T> {
    ...
    private final List<Object> updateValues;
    ...
    protected QueryBuilder(AbstractDao<T, ?> dao) {
        ...
        updateValues = new ArrayList<Object>();
    ...
    public Query<T> build() {
        ...
        if (joinBuilder == null || joinBuilder.length() == 0) {
            select = IQueryDaoAccess.InternalQueryDaoAccess.getStatements(dao).getSelectAll();
        } else {
    ...
    public GroupQuery<T> buildGroup(List<Property> properties, List<FuncProperty> funcProperties) {
        ...
        if (funcProperties != null) {
            ...
            while (iter.hasNext()) {
                ...
                funcBuilder.append(iter.next().getColumn());
            }
        }

        String baseSql = SqlUtils.createSqlSelect(tablename, columns, funcBuilder.toString());
...
//        sql = sql.replace(tablePrefix + ".'", tablename + ".'");
    ...
    /**
     * Builds a reusable query object for update rows (Query objects can be executed more efficiently than creating a
     * QueryBuilder for each execution.
     */
    public UpdateQuery<T> buildUpdate(WhereCondition.UpdateCondition... conditions) {
        String tablename = dao.getTablename();
        String baseSql = SqlUtils.createSqlUpdate(tablename);
        StringBuilder updateBuilder = new StringBuilder();


        appendUpdateClause(updateBuilder, null, conditions);
        StringBuilder builder = new StringBuilder(baseSql);
...
//        sql = sql.replace(tablePrefix + ".'", tablename + ".'");
        ...
        return UpdateQuery.create(dao, sql, updateValues.toArray());
    }
    ...
    public FuncQuery<T> buildFunc(FuncProperty funcProperty) {
        String tablename = dao.getTablename();
        String baseSql = SqlUtils.createSqlSelectRaw(tablename,funcProperty.getColumn());
        StringBuilder builder = new StringBuilder(baseSql);
        appendWhereClause(builder);
        String sql = builder.toString();
        ...
        return FuncQuery.create(dao, sql, values.toArray());
    }
    ...
    private void appendUpdateClause(StringBuilder builder, String tablePrefixOrNull, WhereCondition.UpdateCondition... conditions) {
        updateValues.clear();

        List<WhereCondition.UpdateCondition> conditionList = new ArrayList<WhereCondition.UpdateCondition>();
        for (WhereCondition.UpdateCondition item : conditions) {
            conditionList.add(item);
        }

        if (!conditionList.isEmpty()) {
            ListIterator<WhereCondition.UpdateCondition> iter = conditionList.listIterator();
            while (iter.hasNext()) {
                if (iter.hasPrevious()) {
                    builder.append(" , ");
                }
                WhereCondition condition = iter.next();
                condition.appendTo(builder);
                condition.appendValuesTo(updateValues);
            }
        } else {
