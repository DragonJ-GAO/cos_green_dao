...
<application> element:
    ...
    <provider
            android:name="${contentProvider.javaPackage}.<#if (contentProvider.className)??>${contentProvider.className}<#else>${entity.className}ContentProvider</#if>"
            android:authorities="${contentProvider.authority}"/>
    */

    public class <#if (contentProvider.className)??>${contentProvider.className}<#else>${entity.className}ContentProvider</#if> extends ContentProvider {
        ...
        switch (uriType) {
            ...
            case ${entity.className?upper_case}_ID:
                ...
                if (TextUtils.isEmpty(selection)) {
                    ...
                    rowsUpdated = db.update(TABLENAME, values, PK + "=" + id + " and " + selection, selectionArgs);
                }
                break;
            default:
                throw new IllegalArgumentException("Unknown URI: " + uri);
